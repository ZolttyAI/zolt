...
and tool interactions for the z1 stack (Python, TypeScript, JavaScript, Next.js, NestJS).
"""
import os
import sys
import json
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            try:
                ...
                if status == 429 or status >= 500:
                    ...
                    print(f"[z1-distill] HTTP {status} (attempt {attempt}/{self.max_retries}). Retrying in {delay:.1f}s...")
                    time.sleep(delay)
                else:
                    raise RuntimeError(f"Teacher API error HTTP {status}: {err_body}")
            except (urllib.error.URLError, TimeoutError) as e:
                delay = (2 ** attempt) + random.uniform(0.5, 1.5)
                print(f"[z1-distill] Network error ({e}) (attempt {attempt}/{self.max_retries}). Retrying in {delay:.1f}s...")
                time.sleep(delay)
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        for i in range(n_samples):
            ...
            if count % 10 == 0 or count == n_samples:
                print(f"[z1-distill] Generated {count}/{n_samples} synthetic instances -> {out_path}")
...
def mix_datasets(
    ...
    print(f"[z1-distill] Dataset mixed -> {output_path} | stats: {stats}")
    return stats


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="z1 Teacher Distillation")
