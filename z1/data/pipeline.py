...
def run_step(label: str, fn, *args, **kwargs):
    print(f"\n{'─'*60}")
    print(f"[z1-pipeline] {label}")
    print(f"{'─'*60}")
    result = fn(*args, **kwargs)
    print(f"[z1-pipeline] ✓ {label} complete")
    return result
...
def step_filter_all(raw_dir: str, filtered_dir: str, max_records: int = None):
    ...
    if not jsonl_files:
        print(f"[z1-pipeline] No JSONL files found in {raw_dir}")
        return {}
    ...
    print(f"\n[z1-pipeline] TOTAL: {total_stats['accepted']:,} / {total_stats['total']:,} documents accepted")
    return total_stats


def step_train_tokenizer(filtered_dir: str, tokenizer_out: str, vocab_size: int = 32000):
    """Train BPE tokenizer on filtered dataset."""
    from z1.tokenizer.train_tokenizer import train_z1_tokenizer
    import glob
...
def step_tokenize_all(filtered_dir: str, tokenizer_path: str, tokens_dir: str):
    ...
    from z1.data.filter_code import tokenize_and_save
    ...
    for jf in sorted(filtered_dir.glob("*.jsonl")):
        ...
        if out_bin.exists():
            print(f"[z1-pipeline] Already exists: {out_bin} - skipping")
            continue
        print(f"[z1-pipeline] Tokenize: {jf.name} -> {out_bin.name}")
...
def step_validate_tokens(tokens_dir: str, max_seq_len: int = 4096):
    ...
    if not bin_files:
        print(f"[z1-pipeline] No .bin files found in {tokens_dir}")
        return
    ...
    print(f"\n[z1-pipeline] Total tokens: {total_tokens:,} ({total_tokens/1e9:.2f}B)")

    # Chinchilla compute-optimal target: ~20 tokens per parameter (~3B tokens for 125M model)
    ...
    print(f"[z1-pipeline] Minimum target: 3B tokens | Status: {status}")

    return total_tokens


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="z1 Data Pipeline")
    ...
    p_tok.add_argument("--output", default="z1_tokenizer.json")
    ...
    p_tokenize.add_argument("--tokenizer", default="z1_tokenizer.json")
